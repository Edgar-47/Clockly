...
## 7. Flutter web / cliente móvil

- [ ] Build con `CLOCKLY_API_BASE_URL` apuntando al dominio real en HTTPS:
  ```bash
  flutter build web \
    --dart-define=CLOCKLY_API_BASE_URL=https://tudominio.com/api/v1
  ```
- [ ] Assets de iconos PWA verificados (192 px, 512 px, maskable)
- [ ] `manifest.json` con `start_url` y `scope` correctos
- [ ] Service worker y caché PWA probados en Chrome/Safari
